...
Public Delegate Sub AddToDelegate(ByVal body As LibraryDAL.SMPPDNStrc)
Public Class SMPPInsertDN
    ...
    Private Sub StartListen()
        ...
        _queue.Formatter = New XmlMessageFormatter(New System.Type() {GetType(LibraryDAL.SMPPDNStrc)})
    ...
    Private Sub OnPeekCompleted(ByVal sender As Object, ByVal e As PeekCompletedEventArgs)
        ...
        trans.Commit()
        del.Invoke(CType(msg.Body, LibraryDAL.SMPPDNStrc))
        StartListening()
    End Sub

    Private Sub send2DB(ByVal body As LibraryDAL.SMPPDNStrc)
        Try

            Dim SMPPDNCRUD As New LibraryDAL.SMPPDNCRUD
            SMPPDNCRUD.SMPPDNInsert(body)
