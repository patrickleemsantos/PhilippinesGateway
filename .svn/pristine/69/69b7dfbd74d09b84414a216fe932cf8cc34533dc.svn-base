...
Public Class EGGInsertDN
    ...
    Private UpdateDN_queue As String = ConfigurationManager.AppSettings("updatedn_queue")
    ...
    Private Sub send2DB(ByVal body As LibraryDAL.EGGDNStrc)
        Try
            ...
            logger.Info("[INFO] DN Match=" & forward_queue & ";" & getObjInfo_DNForward(DTO))

            'sendToQueue(UpdateDN_queue, DTO)

            logger.Info("[INFO] Outbox DN update=" & UpdateDN_queue & ";" & getObjInfo_DNForward(DTO))
    ...
    Public Function getObjInfo_DNForward(ByVal objMessage As ForwardDNType) As String
        Dim myText As String = ""
        myText = "MTID=" & objMessage.msgMTid & ";" & _
        "TelcoID=" & objMessage.TelcoID & ";" & _
        "StatusCode=" & objMessage.status_code & ";" & _
        "DN Date=" & objMessage.DnDate
        Return myText
    End Function
End Class
