                ...
                If UserStatus > 0 Then ' check for user exist
                    If UserStatus = General.UserStatus._Active Then ' if user is active
                        If telcoID = General.TelcoID._Smart Then ' reply already subs message for smart telco only
                            ...
                            'Commented by Patrick 10/28/2013
                            'Description: Double opt-in message will be process by CP
                            ...
                            logger.Info("[REG]DoubleOptInMsg;" & objInfo)
                        End If
                            ...
                            If Not String.IsNullOrEmpty(DTOKeywordMsgSetting.WelcomeMsg) Then
                                If telcoID = General.TelcoID._Sun Then
                                    ...
                                    PostResult = SendToAPI(APIUrl, TimeOutPostToAPI, DTOCP.UserName, DTOCP.Password, MSISDN, shortcode, CStr(DTOKeywordMsgSetting.MsgType), _
                                                DTOKeywordMsgSetting.WelcomeMsg, "200", telcoID, KeywordInfo.KeywordID.ToString, "Welcome", TransactionID)
                                    logger.Info("[REG]WelcomeMsg;" & objInfo & ";" & PostResult)
                                Else
                                    'send welcome message
                                    'No need to send welcome message for Smart Feng Shui (it will be handled by CMS)-Patrick 09/27/2013                               
                                    'Added PLQ & Lotto for not sending welcome message-Patrick 11/16/2013
                                    'Added GMOBI for not sending welcome message-Patrick 01/21/2014
                                    If (KeywordInfo.KeywordID <> 68) And (KeywordInfo.KeywordID <> 69) And (KeywordInfo.KeywordID <> 73) And (KeywordInfo.KeywordID <> 76) And (KeywordInfo.KeywordID <> 80) _
                                        And (KeywordInfo.KeywordID <> 81) And (KeywordInfo.KeywordID <> 86) And (KeywordInfo.KeywordID <> 87) And (KeywordInfo.KeywordID <> 92) And (KeywordInfo.KeywordID <> 93) _
                                        And (KeywordInfo.KeywordID <> 98) And (KeywordInfo.KeywordID <> 99) And (KeywordInfo.KeywordID <> 104) And (KeywordInfo.KeywordID <> 117) And (KeywordInfo.KeywordID <> 120) _
                                        And (KeywordInfo.KeywordID <> 123) And (KeywordInfo.KeywordID <> 111) And (KeywordInfo.KeywordID <> 112) And (KeywordInfo.KeywordID <> 126) Then
