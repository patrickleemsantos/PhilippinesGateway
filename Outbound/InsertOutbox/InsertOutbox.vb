...
Public Class InsertOutbox
    ...
    Private Sub insert2EggOutbox(ByVal objMessage As LibraryDAL.Tbl_Outbox)
        ...
        Try
            ...
            Dim sQuery As String = "INSERT INTO premium_sms_cp.mt_egg_" & Format(Now, "yyyyMM") & "_" & GetWeekOfMonth(Now) & " (id,telco_id,shortcode,cp_id,keyword_id,msisdn,charge,content,content_type,wappush_url,data_coding,retry,mo_id,mt_id,mt_response_code,mt_error_code,mt_error_desc,timestamp,dn_code,dn_desc,dn_timestamp,remark) VALUES (0,'" & objMessage.TelcoID & "','" & objMessage.ShortCode & "','" & objMessage.CPID & "','" & objMessage.KeywordID & "','" & objMessage.MSISDN & "','" & objMessage.Charge & "','" & objMessage.Message & "','" & objMessage.Message.Replace("'", "") & "','1','0','" & objMessage.RetryCount & "','" & objMessage.MsgGUID & "','" & objMessage.TransactionID & "','" & objMessage.StatusCode & "','','',now(),'','','','')"
